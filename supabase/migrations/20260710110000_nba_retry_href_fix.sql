...
-- NBA Retry href fix
--
-- The "Retry the failed extraction run" action previously linked
-- to /intake, where there is no "Run pipeline" button. The operator
-- arrived at a dead end. This migration changes that href to
-- /intelligence-layer, where the runPipeline action lives.
--
-- Also updates the "Investigate stalled extraction run" href for
-- consistency — operators should investigate on the intelligence-layer
-- page, not the intake page.
--
-- All other branches are preserved verbatim from the latest version of
-- the function (20260710100000_nba_build_qa_coverage.sql).
-- ============================================================
...
BEGIN
  ...
  -- FIX: Changed href from /intake to /intelligence-layer so the operator
  -- lands on the page with the "Run pipeline" button when retrying.
  IF run_id IS NOT NULL AND run_status = 'failed' THEN
    RETURN QUERY SELECT 'Retry the failed extraction run'::text,
      ...
      ('/engine/projects/' || _project_id || '/intelligence-layer')::text, 'critical'::text; RETURN;
  END IF;

  -- FIX: Also updated the stalled-run href to intelligence-layer for consistency.
