-- NBA coverage extension: Build Execution, OpenClaw runs, QA Evidence.
--
-- This migration extends compute_engine_next_best_action to surface
-- build packet failures, in-flight OpenClaw runs, pending QA evidence
-- reviews, and delivery-readiness nudges. All existing branches are
-- preserved verbatim; new branches are inserted after the review-items
-- branch and before the version-status branches (per priority order).
--
-- Tables referenced by new branches:
--   public.engine_project_build_packets   (status, packet_type)
--   public.engine_project_openclaw_runs   (status, project_id via build_packet_id)
--   public.engine_project_qa_evidence_reviews (status, verdict, project_id)
--   public.engine_roadmap_versions        (already used — reused for delivery check)
--   public.client_portal_roadmaps         (already used — reused for delivery check)
...
DECLARE
  ...
  -- Build execution / OpenClaw / QA locals
  failed_packets int := 0;
  active_packets int := 0;
  failed_oc_runs int := 0;
  running_oc_runs int := 0;
  pending_qa int := 0;
  critical_qa int := 0;
...
BEGIN
  ...
  -- -----------------------------------------------------------------------
  -- NEW BRANCH A: Build packets needing attention
  -- Check engine_project_build_packets for failure or active states.
  -- Valid statuses: draft | ready | handed_off | in_progress | returned |
  --                 qa_required | accepted | rejected | archived
  -- "returned" means the builder returned work needing rework — operator must act.
  -- "handed_off" / "in_progress" mean work is live and in flight.
  -- -----------------------------------------------------------------------
  SELECT
    count(*) FILTER (WHERE status IN ('rejected', 'returned')),
    count(*) FILTER (WHERE status IN ('in_progress', 'handed_off'))
  INTO failed_packets, active_packets
  FROM public.engine_project_build_packets
  WHERE project_id = _project_id;

  IF failed_packets > 0 THEN
    RETURN QUERY SELECT
      'Retry failed build packet'::text,
      failed_packets || ' build packet' || CASE WHEN failed_packets = 1 THEN '' ELSE 's' END || ' was rejected or returned — review and resubmit.',
      ('/engine/projects/' || _project_id || '/build')::text,
      'critical'::text;
    RETURN;
  END IF;

  IF active_packets > 0 THEN
    RETURN QUERY SELECT
      'Build in progress'::text,
      active_packets || ' build packet' || CASE WHEN active_packets = 1 THEN ' is' ELSE 's are' END || ' currently active — check build status.',
      ('/engine/projects/' || _project_id || '/build')::text,
      'info'::text;
    RETURN;
  END IF;
  ...
  -- -----------------------------------------------------------------------
  -- NEW BRANCH B: OpenClaw runs
  -- Join openclaw runs through build packets back to the project.
  -- -----------------------------------------------------------------------
  SELECT
    count(*) FILTER (WHERE ocr.status = 'failed'),
    count(*) FILTER (WHERE ocr.status IN ('running', 'queued', 'sent'))
  INTO failed_oc_runs, running_oc_runs
  FROM public.engine_project_openclaw_runs ocr
  JOIN public.engine_project_build_packets bp ON bp.id = ocr.build_packet_id
  WHERE bp.project_id = _project_id;

  IF failed_oc_runs > 0 THEN
    RETURN QUERY SELECT
      'Review failed OpenClaw run'::text,
      failed_oc_runs || ' OpenClaw run' || CASE WHEN failed_oc_runs = 1 THEN '' ELSE 's' END || ' failed — review output and retry or escalate.',
      ('/engine/projects/' || _project_id || '/build')::text,
      'critical'::text;
    RETURN;
  END IF;

  IF running_oc_runs > 0 THEN
    RETURN QUERY SELECT
      'OpenClaw build in progress'::text,
      running_oc_runs || ' OpenClaw run' || CASE WHEN running_oc_runs = 1 THEN ' is' ELSE 's are' END || ' currently active — no action needed yet.',
      ('/engine/projects/' || _project_id || '/build')::text,
      'info'::text;
    RETURN;
  END IF;
  ...
  -- -----------------------------------------------------------------------
  -- NEW BRANCH C: QA Evidence pending
  -- Check engine_project_qa_evidence_reviews for in_review / draft states
  -- and for critical verdicts (insufficient / needs_owner_decision).
  -- -----------------------------------------------------------------------
  SELECT
    count(*) FILTER (WHERE status IN ('draft', 'in_review')),
    count(*) FILTER (WHERE verdict IN ('insufficient', 'needs_owner_decision') AND status NOT IN ('approved', 'archived', 'rejected'))
  INTO pending_qa, critical_qa
  FROM public.engine_project_qa_evidence_reviews
  WHERE project_id = _project_id;

  -- Critical QA findings take precedence over generic pending reviews.
  IF critical_qa > 0 THEN
    RETURN QUERY SELECT
      'Address critical QA findings before delivery'::text,
      critical_qa || ' QA evidence review' || CASE WHEN critical_qa = 1 THEN '' ELSE 's' END || ' flagged as insufficient or needing a decision.',
      ('/engine/projects/' || _project_id || '/build/qa')::text,
      'critical'::text;
    RETURN;
  END IF;

  IF pending_qa > 0 THEN
    RETURN QUERY SELECT
      'Review QA evidence'::text,
      pending_qa || ' QA evidence review' || CASE WHEN pending_qa = 1 THEN ' is' ELSE 's are' END || ' pending approval.',
      ('/engine/projects/' || _project_id || '/build/qa')::text,
      'warning'::text;
    RETURN;
  END IF;
  ...
  -- -----------------------------------------------------------------------
  -- NEW BRANCH D: Delivery readiness
  -- If all build/QA gates are clear and there is an accepted build packet
  -- but no delivery record yet, nudge the operator to prepare delivery.
  -- Only fires if there are actually packets in this project (otherwise the
  -- project is still in roadmap/planning phase and the version branches below
  -- are more relevant).
  -- -----------------------------------------------------------------------
  SELECT
    count(*) FILTER (WHERE status = 'accepted'),
    count(*)
  INTO accepted_packets, total_packets
  FROM public.engine_project_build_packets
  WHERE project_id = _project_id;

  IF total_packets > 0 AND accepted_packets > 0 THEN
    -- Check if a delivery has already been made via the engine_projects.delivery JSONB.
    -- We reuse proj_name query scope; just check the delivery column inline.
    IF NOT EXISTS (
      ...
      RETURN QUERY SELECT
        'Prepare delivery package'::text,
        accepted_packets || ' build packet' || CASE WHEN accepted_packets = 1 THEN '' ELSE 's' END || ' accepted — all gates clear. Ready to assemble and deliver.',
  ...
  -- -----------------------------------------------------------------------
  -- Version-status branches MOVED UP (from original): an approved or
  -- AI-drafted version should drive the next move even if source/signal
  -- state is stale. (Unchanged from prior migration.)
  -- -----------------------------------------------------------------------
  ...
  -- Source/signal branches only reached if no version exists yet. (Unchanged.)
