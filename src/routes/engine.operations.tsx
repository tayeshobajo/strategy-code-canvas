...
function CreateAgentWizard({ onClose, onCreate }: { onClose: () => void; onCreate: (a: Agent) => void }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4" onClick={onClose}>
      <div className="bg-card rounded-xl border border-border shadow-lg max-w-xl w-full" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Castle Vineyard"
              ...
              <div>
                ...
                <div className="mt-1 space-y-1.5">
                  {TEMPLATES.map((t) => (
                    <label key={t.id} className={cn("flex items-start gap-2 border rounded-md p-2.5 cursor-pointer hover:bg-paper-soft", template === t.id ? "border-royal bg-royal/5" : "border-border")}>
                      <input type="radio" name="tpl" checked={template === t.id} onChange={() => setTemplate(t.id)} className="mt-1" />
          ...
          {step === 2 ? (
            <div className="space-y-2">
              ...
              {MODELS.map((m) => (
                <label key={m.id} className={cn("flex items-start gap-2 border rounded-md p-2.5 cursor-pointer hover:bg-paper-soft", model === m.id ? "border-royal bg-royal/5" : "border-border")}>
                  ...
                  <div>
                    ...
                    <div className="text-xs text-ink/60 font-mono">{m.cost}</div>
                  </div>
                </label>
              ))}
            </div>
          ...
          {step === 4 ? (
            <div className="space-y-3">
              <div>
                ...
                <div className="mt-1 space-y-1.5">
                  {POLICIES.map((p) => (
                    <label key={p} className={cn("flex items-start gap-2 border rounded-md p-2.5 cursor-pointer hover:bg-paper-soft", policy === p ? "border-royal bg-royal/5" : "border-border")}>
                      <input type="radio" name="pol" checked={policy === p} onChange={() => setPolicy(p)} className="mt-1" />
                      <div>
                        <div className="text-sm font-medium text-ink">{p}</div>
                        <div className="text-xs text-ink/60">
                          {p === "Draft only" ? "Agent produces drafts. Nothing takes effect without you." :
                            p === "Propose updates" ? "Agent can push proposed changes into the review queue." :
                              "Agent can execute pre-approved action types. Highest trust — review carefully."}
                        </div>
                      </div>
                    </label>
                  ))}
        ...
        <footer className="flex justify-between p-4 border-t border-border">
          ...
          {step < 4 ? (
            <button
              onClick={() => canNext && setStep((s) => (s + 1) as 2 | 3 | 4)}
              disabled={!canNext}
              className="text-xs px-3 py-1.5 rounded bg-ink text-white hover:bg-ink/90 disabled:opacity-40"
            >
              Continue
            </button>
          ) : (
            <button
              onClick={submit}
              className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded bg-royal text-white hover:bg-royal/90"
            >
              <Check className="w-3 h-3" /> Create agent
            </button>
          )}
        </footer>
      </div>
