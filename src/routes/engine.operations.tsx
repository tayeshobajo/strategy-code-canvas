import { createFileRoute } from "@tanstack/react-router";
import { EmptyState, SectionCard } from "@/components/engine/primitives";

export const Route = createFileRoute("/engine/operations")({
  component: () => (
    <div className="max-w-[1200px]">
      <div className="font-mono text-[11px] uppercase tracking-[0.28em] text-royal">Ops</div>
      <h1 className="font-display text-4xl text-ink mt-1 mb-6">Global Operations</h1>
      <SectionCard title="System operations">
        <EmptyState title="Coming in the next build" hint="Agent health, cost controls, and system operations land here." />
      </SectionCard>
    </div>
  ),
});
