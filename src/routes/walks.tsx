...
function HeroRoute({ inView }: { inView: boolean }) {
  // Ascending dotted blue route climbing lower-left -> upper-right across the
    ...
    >
      <defs>
        <mask id="walks-hero-reveal" maskUnits="userSpaceOnUse">
          <rect width="900" height="360" fill="black" />
          <path
            d={d}
            fill="none"
            stroke="white"
      ...
      >
        ...
        <SummitFlag x={last[0]} y={last[1] - 2} scale={1.2} />
      </g>
    </svg>
  );
}
        ...
        <div className="grid grid-cols-1 gap-10 pb-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] lg:gap-16 lg:pb-16">
          ...
          >
            ...
            <img
              ...
              aria-hidden="true"
              draggable={false}
              className="pointer-events-none absolute -right-16 bottom-0 w-[170%] max-w-none select-none opacity-95 lg:w-[165%]"
            />
