    ...
    ? rec.diagnosis
        .map((item) => {
          ...
          const bullets = Array.isArray(r.bullets)
            ? r.bullets.map((b) => cleanString(b).slice(0, 240)).filter(Boolean).slice(0, 4)
...
function statusMap(rows: TruthRow[]): Map<string, EpistemicStatus> {
  return new Map(rows.map((row) => [row.field_key, row.status]));
}
...
export const fillMissingSpineDetailsFromIntake = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }): Promise<{ ok: true; changed: string[]; statuses: string[] }> => {
    ...
    const { data: project, error: projectErr } = await sb
      ...
      .select(
        "id,name,updated_at,step_states,point_a,point_b,signal_room,extraction,hidden_assets,gap_map,blueprint,roadmap,sequencing,deadlines,investment,client_preview, engine_clients(company,industry,notes)",
      )
    ...
    const stepStates = (project.step_states ?? {}) as Record<string, { state?: string; updated_at?: string; updated_by?: string | null; note?: string | null }>;
    if (stepStates["point-a"]?.state === "approved" || stepStates["point-b"]?.state === "approved") {
      throw new Error("AI Product Manager cannot overwrite an approved Point A or Point B. Reopen the step first.");
    }
    ...
    const pointAStatus = statusMap(((truthRows ?? []) as Array<TruthRow & { spine: string }>).filter((r) => r.spine === "point-a"));
    const pointBStatus = statusMap(((truthRows ?? []) as Array<TruthRow & { spine: string }>).filter((r) => r.spine === "point-b"));

    const contextPayload = {
      project: {
        name: project.name,
        client: project.engine_clients ?? null,
      },
      current_modules: {
      ...
      sources: (sourceRows ?? []).map((s: any) => ({
        ...
        type: s.type,
        status: s.status,
        text: typeof s.raw_text === "string" ? s.raw_text.slice(0, 6_000) : "",
      })),
      missing_content: {
        point_a: POINT_A_BASE_FIELD_KEYS.filter((key) => isBlank((existingPointA as Record<string, unknown>)[key])),
      ...
      [
        {
          role: "system",
          content:
            "You are the Trust Tai AI Product Manager. Fill missing Project Spine details from intake, extracted signals, and current project modules. Do not approve anything. Do not claim certainty when the source is thin. If a detail is inferred, phrase it as a reviewable draft. No em dashes, no exclamation points, no generic SaaS filler. Return strict JSON only.",
        },
        {
          role: "user",
          content: `Use the project context below to draft missing Point A and Point B details.

Rules:
- Preserve existing content. Only provide replacement content for blank or incomplete fields.
- Point A lenses should be 4 to 6 business lenses with label, value, hint.
- Point A diagnosis should be 3 to 6 cards with title, tag, bullets. Tags may be PARTIAL, LIMITED, MISSING, RISK, CONSTRAINT, DEFAULT.
- Point A key_diagnosis should be one concise paragraph.
- Point B must use exactly these keys: ${POINT_B_FIELD_KEYS.join(", ")}.
- If the source does not prove the answer, write a useful draft and make clear it needs confirmation.

Return JSON:
{
  "point_a": {
    "lenses": [{"label":"","value":"","hint":""}],
    "diagnosis": [{"title":"","tag":"","bullets":[""]}],
    "key_diagnosis": ""
  },
  "point_b": {
  ...
  "summary": "one sentence summary of what was drafted"
}
...
${JSON.stringify(contextPayload, null, 2).slice(0, 45_000)}`,
    ...
    const canWritePointA = (key: string) => !HUMAN_LOCKED_STATUSES.has(pointAStatus.get(key) as EpistemicStatus);
    const canWritePointB = (key: string) => !HUMAN_LOCKED_STATUSES.has(pointBStatus.get(key) as EpistemicStatus);

    if (isBlank(nextPointA.lenses) && draftPointA.lenses?.length && canWritePointA("lenses")) {
      nextPointA.lenses = draftPointA.lenses;
      changed.push("point_a.lenses");
    }
    if (isBlank(nextPointA.diagnosis) && draftPointA.diagnosis?.length && canWritePointA("diagnosis")) {
      nextPointA.diagnosis = draftPointA.diagnosis;
      changed.push("point_a.diagnosis");
    }
    if (isBlank(nextPointA.key_diagnosis) && draftPointA.key_diagnosis && canWritePointA("key_diagnosis")) {
      nextPointA.key_diagnosis = draftPointA.key_diagnosis;
      changed.push("point_a.key_diagnosis");
    }

    for (const key of POINT_B_FIELD_KEYS) {
      if (isBlank(nextPointB[key]) && draftPointB[key] && canWritePointB(key)) {
    ...
    const prevA = asRecord(project.point_a);
    const nextARecord = nextPointA as Record<string, unknown>;
    if (changedKeys(prevA, nextARecord).length) patch.point_a = nextPointA;
    ...
    if (Object.keys(patch).length) {
      ...
      if (updateErr) throw new Error((updateErr as { message?: string }).message ?? "Failed to save AI-filled Spine details");
    }
    ...
    for (const card of nextPointA.diagnosis ?? []) {
      ...
      if (key.trim() !== "diagnosis:" && !HUMAN_LOCKED_STATUSES.has(pointAStatus.get(key) as EpistemicStatus)) {
    ...
    if (truthWrites.length) {
      ...
      if (truthErr) throw new Error((truthErr as { message?: string }).message ?? "Failed to save durable truth statuses");
    }
    ...
    if (auditRows.length) await sb.from("engine_audit_log").insert(auditRows);
    ...
    return { ok: true, changed, statuses: truthWrites.map((row) => `${row.spine}.${row.field_key}`) };
  });